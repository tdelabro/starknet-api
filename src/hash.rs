...
impl StarkFelt {
    ...
    /// Returns a new unckecked [`StarkFelt`].
    ///
    /// # Safety
    /// The most significant byte nible of the array must be 0 (`byte[0] < 0x10`).
    /// Otherwise the resulting [`StarkFelt`] will end up containing an out of range value.
    pub const unsafe fn new_unchecked(bytes: [u8; 32]) -> Self {
        Self(bytes)
    }
